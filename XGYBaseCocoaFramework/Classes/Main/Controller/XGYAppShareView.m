...
@implementation XGYAppShareView
...
- (void)shareImageViewTouchAction:(UIButton *)sender
{
    ...
    if (!self.shareURL) {
        [XGYMessageTool showMessage:@"分享链接不能为空！"];
        return;
    }
    ...
    [ShareSDK share:type parameters:shareParams onStateChanged:^(SSDKResponseState state, NSDictionary *userData, SSDKContentEntity *contentEntity, NSError *error) {
        ...
        switch (state) {
            ...
            case SSDKResponseStateSuccess:
            {
                ...
                [XGYMessageTool showMessage:@"分享成功"];
            ...
            {
                [XGYMessageTool showMessage:@"分享失败，请检查网络连接~"];
            }
                break;
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    if ([view isKindOfClass:[XGYAppShareView class]]) {
        [self hiddeMenueViewWithAnimation];
    }
}
@end
