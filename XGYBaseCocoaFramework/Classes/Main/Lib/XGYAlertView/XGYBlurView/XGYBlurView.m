...
@interface XGYBlurScheduler : NSObject
...
@interface XGYBlurLayer: CALayer
...
@implementation XGYBlurScheduler

+ (instancetype)sharedInstance
{
    static XGYBlurScheduler *sharedInstance = nil;
    if (!sharedInstance)
    {
        sharedInstance = [[XGYBlurScheduler alloc] init];
    }
...
- (void)setBlurEnabled:(BOOL)blurEnabled
{
    ...
    if (blurEnabled)
    {
        for (XGYBlurView *view in self.views)
        {
...
- (void)removeView:(XGYBlurView *)view
{
...
@implementation XGYBlurView

@synthesize underlyingView = _underlyingView;

+ (void)setBlurEnabled:(BOOL)blurEnabled
{
    [XGYBlurScheduler sharedInstance].blurEnabled = blurEnabled;
}

+ (void)setUpdatesEnabled
{
    [[XGYBlurScheduler sharedInstance] setUpdatesEnabled];
}

+ (void)setUpdatesDisabled
{
    [[XGYBlurScheduler sharedInstance] setUpdatesDisabled];
}

+ (Class)layerClass
{
    return [XGYBlurLayer class];
}
...
- (XGYBlurLayer *)blurLayer
{
    return (XGYBlurLayer *)self.layer;
}

- (XGYBlurLayer *)blurPresentationLayer
{
    XGYBlurLayer *blurLayer = [self blurLayer];
    return (XGYBlurLayer *)blurLayer.presentationLayer ?: blurLayer;
}
...
- (void)schedule
{
    ...
    {
        [[XGYBlurScheduler sharedInstance] addView:self];
    }
    else
    {
        [[XGYBlurScheduler sharedInstance] removeView:self];
    }
}
...
- (BOOL)shouldUpdate
{
    ...
    self.blurEnabled && [XGYBlurScheduler sharedInstance].blurEnabled &&
...
- (UIImage *)snapshotOfUnderlyingView
{
    __strong XGYBlurLayer *blurLayer = [self blurPresentationLayer];
    __strong CALayer *underlyingLayer = [self underlyingLayer];
