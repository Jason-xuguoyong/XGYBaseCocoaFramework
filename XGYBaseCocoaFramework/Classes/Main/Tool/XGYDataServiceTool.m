...
#import "XGYDataServiceTool.h"
#import "XGYBaseHttpTool.h"


@implementation XGYDataServiceTool
...
+ (void)requestSuccessWithData:(id)responseObj Success:(void (^)(id responseObj))success refreshTableView:(UITableView *)tableView isShowHUD:(BOOL)isShowHUD
{
    ...
    [XGYDataServiceTool hiddenHUDAndNetworkActivityIndicatorVisible:tableView isShowHUD:isShowHUD];
...
+ (void)requestFaileWithErrorTask:(NSURLSessionDataTask *) task error:( NSError *)error failure:(void (^)(NSError *error,NSString *errorCode,NSString *remark))failure showError:(BOOL)showError refreshTableView:(UITableView *)tableView showHUD:(BOOL)showHud
{
    ...
    XGYDebugLog(@"error---%@",error);
     [XGYDataServiceTool hiddenHUDAndNetworkActivityIndicatorVisible:tableView isShowHUD:showHud];
    id data = [error.userInfo objectForKey:@"com.alamofire.serialization.response.error.data"];
    NSHTTPURLResponse * responses = (NSHTTPURLResponse *)task.response;
    
    //先处理请求超时的情况
    if (error.code == -1001/*请求超时*/) {
        [XGYMessageTool showMessage:@"当前网络似乎不太给力~"];
    ...
    if (data) {
        ...
        if (responses.statusCode == 401/*需要回到登录页面*/) {
            [XGYMessageTool showMessage:@"请先登录"];
        }else if(responses.statusCode == 400){
            [XGYMessageTool showMessage:meg];
        }else if(responses.statusCode == 404){
            [XGYMessageTool showMessage:[NSString stringWithFormat:@"服务器充电中..."]];
        }else if(responses.statusCode == 403){
            [XGYMessageTool showMessage:[NSString stringWithFormat:@"服务器上火星去啦~"]];
        }else if(responses.statusCode == 402){
            [XGYMessageTool showMessage:[NSString stringWithFormat:@"服务器迷路咯~"]];
        }else if(responses.statusCode == 500){
            [XGYMessageTool showMessage:@"我们的攻城狮正在拼命修复..."];
        }else if(responses.statusCode == 405){
            [XGYMessageTool showMessage:@"服务器找不到请求方向..."];
        }else
        {
            XGYDebugLog(@"请求失败，后台没有返回原因error=%@",error);
        }
...
{
    // 1.没网不清求
    if (![XGYDataServiceTool checkNetworkingRefreshTableView:tableView failure:failure]) {
        return;
    }
   
    // 2.获取请求的全路径 ，并处理请求前UI
    NSString *allURL = [XGYDataServiceTool getAllUrlString:url showHUD:showHud parma:parma];
    ...
    switch (method) {
        ...
        {
            [XGYBaseHttpTool postDataWithURL:allURL parma:parma isJason:isJason showErrorMessage:showError showHUD:showHud refreshTableView:tableView success:^(id responseObj) {
                
                // 成功
                [XGYDataServiceTool requestSuccessWithData:responseObj Success:success refreshTableView:tableView isShowHUD:showHud];
                
            } failure:^(NSURLSessionDataTask *task, NSError *error) {
                //失败
                [XGYDataServiceTool requestFaileWithErrorTask:task error:error failure:failure showError:showError refreshTableView:tableView showHUD:showHud];
            }];
        ...
        {
            [XGYBaseHttpTool getDataWithURL:allURL parma:parma isJason:isJason showErrorMessage:showError showHUD:showHud refreshTableView:tableView success:^(id responseObj) {
        ...
        {
            [XGYBaseHttpTool delectDataWithURL:allURL parma:parma isJason:isJason showErrorMessage:showError showHUD:showHud refreshTableView:tableView success:^(id responseObj) {
                // 成功
                [XGYDataServiceTool requestSuccessWithData:responseObj Success:success refreshTableView:tableView isShowHUD:showHud ];
        ...
        {
            [XGYBaseHttpTool putDataWithURL:allURL parma:parma isJason:isJason showErrorMessage:showError showHUD:showHud refreshTableView:tableView success:^(id responseObj) {
...
+ (BOOL)checkNetworkingRefreshTableView:(UITableView *)tableView failure:(void (^)(NSError *error,NSString *errorCode,NSString *remark))failure
{
    ...
    if (![XGYMessageTool isHaveNetWork]) {
        ...
        [XGYMessageTool showMessage:@"世界上最遥远的距离就是没有网络~"];
        [XGYDataServiceTool hiddenHUDAndNetworkActivityIndicatorVisible:tableView isShowHUD:YES];

    }
    return [XGYMessageTool isHaveNetWork];
}
