...
@interface RMTControllerTool ()
@end


@implementation RMTControllerTool
...
{
    RMTAppShareView *share = [[RMTAppShareView alloc] initWithShareURLString:shareURL shareTitle:title shareConten:shareConten shareIcon:icon];
...
+ (void)chooseRootViewController
{
    ...
    window.rootViewController = [[RMTTabBarController alloc] init];
...
/** ActionSheet***/
 ...
 /***
 [RMTControllerTool showActionSheetWithTitle:@"请选择图片来源" cancelButtonTitle:@"取消" buttonTitleArray:@[@"拍摄",@"从相册选择"] destructiveButtonIndex:0 cancelButtonClick:^(UIButton *clickButton) {
 RMTDebugLog(@"点击取消按钮");
 } buttonClickBlock:^(NSString *clickButtonTitle, int buttonIndex) {
 RMTDebugLog(@"当前点击的标题是%@，所在数组下表是%d",clickButtonTitle,buttonIndex);
 }];
...
/***
 [RMTControllerTool showAlertViewWithTitle:@"是否确认退出登录?" message:@"退出登陆" leftButtonTitle:@"确定" rightButtonTitle:@"取消" leftButtonClickBlock:^(UIButton *sender) {
 RMTDebugLog(@"右边按钮的点击事件");
 } rightButtonClickBlock:^(UIButton *sender) {
 RMTDebugLog(@"左边按钮的点击事件");
 }];
...
/***
    [RMTControllerTool showShareViewWithShareURLString:@"http://www.baidu.com"];
